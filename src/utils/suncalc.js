...
SunCalc.getPosition = getPosition;

SunCalc.toJulian = toJulian;
...
export const getTimes = (date, lat, lng, height = 0) => {
  const lw = rad * -lng,
    phi = rad * lat,
...
SunCalc.getTimes = getTimes;

// moon calculations, based on astronomical formulas
...
SunCalc.getMoonPosition = (date, lat, lng) => {
  ...
  const hCorrected = h + astroRefraction(h);
